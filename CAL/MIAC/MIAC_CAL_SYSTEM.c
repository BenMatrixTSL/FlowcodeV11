// CRC: 
// REVISION: 1.0
// GUID: 8B00F32A-AF71-4929-9FC3-D0EC0AD18444
// DATE: 08\08\2022
// DIR: CAL\MIAC\MIAC_CAL_SYSTEM.c
/*********************************************************************
 *                  Flowcode CAL MIAC System File
 *
 * File: MIAC_CAL_SYSTEM.c
 *
 * (c) 2016 Matrix TSL
 ...
 *  date  | by | description
 * -------+----+-----------------------------------------------------
 * 270516 | LM | Created from PIC_CAL_MIAC_SYSTEM.c modified for V3 API
 * 
 */
...
MX_UINT8 CAN_SPI_SendByte(MX_UINT8);
...
void CAN_SPI_Init()
{
#define MX_SYSTEM_CAN_SPI_MODE 0
	...
	SPI2CON1bits.CKE = (MX_SYSTEM_CAN_SPI_MODE & 0x01)?0:1;	// Assign Clock Phase (1=out data changes on edge active to idle clock, i.e. falling)
	SPI2CON1bits.CKP = (MX_SYSTEM_CAN_SPI_MODE & 0x02)?1:0;	// Assign Clock Polarity (0=idle state is low)
	...
	if (MX_SYSTEM_CAN_SPI_MODE & 0x01) SPCR |= (1 << CPHA);	// Clock Phase
	if (MX_SYSTEM_CAN_SPI_MODE & 0x02) SPCR |= (1 << CPOL);	// Clock Polarity
	SPCR |= (1 << SPE);						// SPI Enable
#endif
}


MX_UINT8 CAN_SPI_SendByte(MX_UINT8 byte)
{
	MX_UINT8 retval;
#if defined (MX_MIAC_MODULE_DSP)
	retval = SPI2BUF;					// dummy read to clear flags
	SPI2BUF = byte;						// send outgoing byte
	while (SPI2STATbits.SPIRBF == 0);	// wait until data has been sent
	retval = SPI2BUF;					// receive the byte
#elif defined (MX_MIAC_MODULE_AVR)
	SPDR = byte;						// send outgoing byte
	while (!(SPSR & (1<<SPIF)));		// wait until data has been sent
	retval = SPDR;						// receive the byte
#endif
	return retval;
}
...
void CAN_Write(MX_UINT8 addr, MX_UINT8 data)
{
	//enable SPI
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_WRITE);			//send a byte
	CAN_SPI_SendByte(addr);					//send the address
	CAN_SPI_SendByte(data);					//send the data
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
}
...
MX_UINT8 CAN_Read(MX_UINT8 addr)
{
	...
	//enable SPI
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_READ);				//read a byte
	CAN_SPI_SendByte(addr);					//send the address
	data = CAN_SPI_SendByte(0x5A);			//send a don't care byte
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
	return (data);
}
...
MX_UINT8 CAN_ReadStatus()
{
	...
	//enable SPI
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_RD_STATUS);		//read the status
	retval = CAN_SPI_SendByte(0x5A);		//send a don't care byte
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
	return (retval);
}
...
MX_UINT8 CAN_ReadRXStatus()
{
	...
	//enable SPI
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_RX_STATUS);		//read the status
	retval = CAN_SPI_SendByte(0x5A);		//send a don't care byte
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
	return (retval);
...
void CAN_BitModify(MX_UINT8 addr, MX_UINT8 mask, MX_UINT8 data)
{
	//enable SPI
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_BIT_MODIFY);		//bit modify
	CAN_SPI_SendByte(addr);					//send the address
	CAN_SPI_SendByte(mask);					//send the mask
	CAN_SPI_SendByte(data);					//send the data
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
}
...
MX_UINT8 CAN_ReadRX_ID_and_Data(MX_UINT8 buffer)
{
	...
	//enable SPI
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_RD_RXBUF_ID(buffer));		//read the RX ID
	CAN_MSG_ID[idx++] = CAN_SPI_SendByte(0x5A);		//read back SIDH
	CAN_MSG_ID[idx++] = CAN_SPI_SendByte(0x5A);		//read back SIDL

	retval = CAN_SPI_SendByte(0x5A);				//send a don't care byte
  #ifdef CAN_USE_EXID
	CAN_MSG_ID[idx++] = retval;						//read back EXID8
  #endif

	retval = CAN_SPI_SendByte(0x5A);				//send a don't care byte
  #ifdef CAN_USE_EXID
	CAN_MSG_ID[idx++] = retval;						//read back EXID0
  #endif

	CAN_MSG_DLC = CAN_SPI_SendByte(0x5A);			//read back DLC

	//now read the data (if there is any)
	if (CAN_MSG_DLC > 0)
	{
		CAN_MSG_DATA[0] = CAN_SPI_SendByte(0x5A);		//read back D0

		if (CAN_MSG_DLC > 1)
		{
			CAN_MSG_DATA[1] = CAN_SPI_SendByte(0x5A);			//read back D1

			if (CAN_MSG_DLC > 2)
			{
				CAN_MSG_DATA[2] = CAN_SPI_SendByte(0x5A);			//read back D2

				if (CAN_MSG_DLC > 3)
				{
					CAN_MSG_DATA[3] = CAN_SPI_SendByte(0x5A);			//read back D3

					if (CAN_MSG_DLC > 4)
					{
						CAN_MSG_DATA[4] = CAN_SPI_SendByte(0x5A);			//read back D4

						if (CAN_MSG_DLC > 5)
						{
							CAN_MSG_DATA[5] = CAN_SPI_SendByte(0x5A);			//read back D5

							if (CAN_MSG_DLC > 6)
							{
								CAN_MSG_DATA[6] = CAN_SPI_SendByte(0x5A);			//read back D6

								if (CAN_MSG_DLC > 7)
								{
									CAN_MSG_DATA[7] = CAN_SPI_SendByte(0x5A);			//read back D7
								}
							}
						}
					}
...
void CAN_Init()
{
	...
	CAN_SPI_SendByte(CAN_WRITE);        //CAN write mode
	CAN_SPI_SendByte(0x00);             //start writting at address 0x00

	CAN_SPI_SendByte(RXF0SIDH_VAL);     // RXF0SIDH  0x00
	CAN_SPI_SendByte(RXF0SIDL_VAL);     // RXF0SIDL  0x01
	CAN_SPI_SendByte(RXF0EID8_VAL);     // RXF0EID8  0x02
	CAN_SPI_SendByte(RXF0EID0_VAL);     // RXF0EID0  0x03
	CAN_SPI_SendByte(RXF1SIDH_VAL);     // RXF1SIDH  0x04
	CAN_SPI_SendByte(RXF1SIDL_VAL);     // RXF1SIDL  0x05
	CAN_SPI_SendByte(RXF1EID8_VAL);     // RXF1EID8  0x06
	CAN_SPI_SendByte(RXF1EID0_VAL);     // RXF1EID0  0x07
	CAN_SPI_SendByte(RXF2SIDH_VAL);     // RXF2SIDH  0x08
	CAN_SPI_SendByte(RXF2SIDL_VAL);     // RXF2SIDL  0x09
	CAN_SPI_SendByte(RXF2EID8_VAL);     // RXF2EID8  0x0A
	CAN_SPI_SendByte(RXF2EID0_VAL);     // RXF2EID0  0x0B
	CAN_SPI_SendByte(BFPCTRL_VAL);      // BFPCTRL   0x0C
	CAN_SPI_SendByte(TXRTSCTRL_VAL);    // TXRTSCTRL 0x0D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x0E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x0F

	CAN_SPI_SendByte(RXF3SIDH_VAL);     // RXF3SIDH  0x10
	CAN_SPI_SendByte(RXF3SIDL_VAL);     // RXF3SIDL  0x11
	CAN_SPI_SendByte(RXF3EID8_VAL);     // RXF3EID8  0x12
	CAN_SPI_SendByte(RXF3EID0_VAL);     // RXF3EID0  0x13
	CAN_SPI_SendByte(RXF4SIDH_VAL);     // RXF4SIDH  0x14
	CAN_SPI_SendByte(RXF4SIDL_VAL);     // RXF4SIDL  0x15
	CAN_SPI_SendByte(RXF4EID8_VAL);     // RXF4EID8  0x16
	CAN_SPI_SendByte(RXF4EID0_VAL);     // RXF4EID0  0x17
	CAN_SPI_SendByte(RXF5SIDH_VAL);     // RXF5SIDH  0x18
	CAN_SPI_SendByte(RXF5SIDL_VAL);     // RXF5SIDL  0x19
	CAN_SPI_SendByte(RXF5EID8_VAL);     // RXF5EID8  0x1A
	CAN_SPI_SendByte(RXF5EID0_VAL);     // RXF5EID0  0x1B
	CAN_SPI_SendByte(0x00);             // TEC       0x1C
	CAN_SPI_SendByte(0x00);             // REC       0x1D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x1E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x1F

	CAN_SPI_SendByte(RXM0SIDH_VAL);     // RXM0SIDH  0x20
	CAN_SPI_SendByte(RXM0SIDL_VAL);     // RXM0SIDL  0x21
	CAN_SPI_SendByte(RXM0EID8_VAL);     // RXM0EID8  0x22
	CAN_SPI_SendByte(RXM0EID0_VAL);     // RXM0EID0  0x23
	CAN_SPI_SendByte(RXM1SIDH_VAL);     // RXM1SIDH  0x20
	CAN_SPI_SendByte(RXM1SIDL_VAL);     // RXM1SIDL  0x21
	CAN_SPI_SendByte(RXM1EID8_VAL);     // RXM1EID8  0x26
	CAN_SPI_SendByte(RXM1EID0_VAL);     // RXM1EID0  0x27
	CAN_SPI_SendByte(CNF3_VAL);         // CNF3      0x28
	CAN_SPI_SendByte(CNF2_VAL);         // CNF2      0x29
	CAN_SPI_SendByte(CNF1_VAL);         // CNF1      0x2A
	CAN_SPI_SendByte(CANINTE_VAL);      // CANINTE   0x2B
	CAN_SPI_SendByte(0x00);             // CANINTF   0x2C
	CAN_SPI_SendByte(0x00);             // EFLG      0x2D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x2E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x2F

	CAN_SPI_SendByte(TXB0CTRL_VAL);     // TXB0CTRL  0x30
	CAN_SPI_SendByte(TXB0SIDH_VAL);     // TXB0SIDH  0x31
	CAN_SPI_SendByte(TXB0SIDL_VAL);     // TXB0SIDL  0x32
	CAN_SPI_SendByte(TXB0EID8_VAL);     // TXB0EID8  0x33
	CAN_SPI_SendByte(TXB0EID0_VAL);     // TXB0EID0  0x34
	CAN_SPI_SendByte(TXB0DLC_VAL);      // TXB0DLC   0x35
	CAN_SPI_SendByte(TXB0DB0_VAL);      // TXB0DB0   0x36
	CAN_SPI_SendByte(TXB0DB1_VAL);      // TXB0DB1   0x37
	CAN_SPI_SendByte(TXB0DB2_VAL);      // TXB0DB2   0x38
	CAN_SPI_SendByte(TXB0DB3_VAL);      // TXB0DB3   0x39
	CAN_SPI_SendByte(TXB0DB4_VAL);      // TXB0DB4   0x3A
	CAN_SPI_SendByte(TXB0DB5_VAL);      // TXB0DB5   0x3B
	CAN_SPI_SendByte(TXB0DB6_VAL);      // TXB0DB6   0x3C
	CAN_SPI_SendByte(TXB0DB7_VAL);      // TXB0DB7   0x3D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x3E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x3F

	CAN_SPI_SendByte(TXB1CTRL_VAL);     // TXB1CTRL  0x40 Highest priority
	CAN_SPI_SendByte(TXB1SIDH_VAL);     // TXB1SIDH  0x41
	CAN_SPI_SendByte(TXB1SIDL_VAL);     // TXB1SIDL  0x42
	CAN_SPI_SendByte(TXB1EID8_VAL);     // TXB1EID8  0x43
	CAN_SPI_SendByte(TXB1EID0_VAL);     // TXB1EID0  0x44
	CAN_SPI_SendByte(TXB1DLC_VAL);      // TXB1DLC   0x45
	CAN_SPI_SendByte(TXB1DB0_VAL);      // TXB1DB0   0x46
	CAN_SPI_SendByte(TXB1DB1_VAL);      // TXB1DB1   0x47
	CAN_SPI_SendByte(TXB1DB2_VAL);      // TXB1DB2   0x48
	CAN_SPI_SendByte(TXB1DB3_VAL);      // TXB1DB3   0x49
	CAN_SPI_SendByte(TXB1DB4_VAL);      // TXB1DB4   0x4A
	CAN_SPI_SendByte(TXB1DB5_VAL);      // TXB1DB5   0x4B
	CAN_SPI_SendByte(TXB1DB6_VAL);      // TXB1DB6   0x4C
	CAN_SPI_SendByte(TXB1DB7_VAL);      // TXB1DB7   0x4D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x4E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x4F

	CAN_SPI_SendByte(TXB2CTRL_VAL);     // TXB2CTRL  0x50
	CAN_SPI_SendByte(TXB2SIDH_VAL);     // TXB2SIDH  0x51
	CAN_SPI_SendByte(TXB2SIDL_VAL);     // TXB2SIDL  0x52
	CAN_SPI_SendByte(TXB2EID8_VAL);     // TXB2EID8  0x53
	CAN_SPI_SendByte(TXB2EID0_VAL);     // TXB2EID0  0x54
	CAN_SPI_SendByte(TXB2DLC_VAL);      // TXB2DLC   0x55
	CAN_SPI_SendByte(TXB2DB0_VAL);      // TXB2DB0   0x56
	CAN_SPI_SendByte(TXB2DB1_VAL);      // TXB2DB1   0x57
	CAN_SPI_SendByte(TXB2DB2_VAL);      // TXB2DB2   0x58
	CAN_SPI_SendByte(TXB2DB3_VAL);      // TXB2DB3   0x59
	CAN_SPI_SendByte(TXB2DB4_VAL);      // TXB2DB4   0x5A
	CAN_SPI_SendByte(TXB2DB5_VAL);      // TXB2DB5   0x5B
	CAN_SPI_SendByte(TXB2DB6_VAL);      // TXB2DB6   0x5C
	CAN_SPI_SendByte(TXB2DB7_VAL);      // TXB2DB7   0x5D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x5E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x5F

	CAN_SPI_SendByte(RXB0CTRL_VAL);     // RXB0CTRL  0x60
	CAN_SPI_SendByte(0x00);             // RXB0SIDH  0x61
	CAN_SPI_SendByte(0x00);             // RXB0SIDL  0x62
	CAN_SPI_SendByte(0x00);             // RXB0EID8  0x63
	CAN_SPI_SendByte(0x00);             // RXB0EID0  0x64
	CAN_SPI_SendByte(0x00);             // RXB0DLC   0x65
	CAN_SPI_SendByte(0x00);             // RXB0DB0   0x66
	CAN_SPI_SendByte(0x00);             // RXB0DB1   0x67
	CAN_SPI_SendByte(0x00);             // RXB0DB2   0x68
	CAN_SPI_SendByte(0x00);             // RXB0DB3   0x69
	CAN_SPI_SendByte(0x00);             // RXB0DB4   0x6A
	CAN_SPI_SendByte(0x00);             // RXB0DB5   0x6B
	CAN_SPI_SendByte(0x00);             // RXB0DB6   0x6C
	CAN_SPI_SendByte(0x00);             // RXB0DB7   0x6D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x6E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x6F

	CAN_SPI_SendByte(RXB1CTRL_VAL);     // RXB1CTRL  0x70
	CAN_SPI_SendByte(0x00);             // RXB1SIDH  0x71
	CAN_SPI_SendByte(0x00);             // RXB1SIDL  0x72
	CAN_SPI_SendByte(0x00);             // RXB1EID8  0x73
	CAN_SPI_SendByte(0x00);             // RXB1EID0  0x74
	CAN_SPI_SendByte(0x00);             // RXB1DLC   0x75
	CAN_SPI_SendByte(0x00);             // RXB1DB0   0x76
	CAN_SPI_SendByte(0x00);             // RXB1DB1   0x77
	CAN_SPI_SendByte(0x00);             // RXB1DB2   0x78
	CAN_SPI_SendByte(0x00);             // RXB1DB3   0x79
	CAN_SPI_SendByte(0x00);             // RXB1DB4   0x7A
	CAN_SPI_SendByte(0x00);             // RXB1DB5   0x7B
	CAN_SPI_SendByte(0x00);             // RXB1DB6   0x7C
	CAN_SPI_SendByte(0x00);             // RXB1DB7   0x7D
	CAN_SPI_SendByte(MX_CANSTAT_VAL);   // MX_CANSTAT   0x7E
	CAN_SPI_SendByte(CANCTRL_VAL);      // CANCTRL   0x7F
...
void CAN_SetTxData(MX_UINT8 buffer, MX_UINT8 data_cnt, MX_UINT8 d0, MX_UINT8 d1, MX_UINT8 d2, MX_UINT8 d3, MX_UINT8 d4, MX_UINT8 d5, MX_UINT8 d6, MX_UINT8 d7)
{
	...
	CAN_SPI_SendByte(CAN_WRITE);        //CAN write mode
	CAN_SPI_SendByte(dlc_addr);         //start writting at address 0x00
	CAN_SPI_SendByte(data_cnt);         //send the DLC value
...
void CAN_SetTxID(MX_UINT8 buffer, MX_UINT16 ID)
{
	...
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_WRITE);		//CAN write mode
	CAN_SPI_SendByte(id_addr);			//start writting at hi id address
	CAN_SPI_SendByte(ID);				//send the Hi ID value
	CAN_SPI_SendByte(ID_L);				//send the Lo ID value
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
}
...
void CAN_SetFilter (MX_UINT16 ID)
{
	...
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_WRITE);       	//CAN write mode
	CAN_SPI_SendByte(0x00);				//start writting at address 0x00 (RXF0SIDH)
	CAN_SPI_SendByte(ID);     			// RXF0SIDH  0x00
	CAN_SPI_SendByte(ID_L);				// RXF0SIDL  0x01
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
	//Put the CAN device into normal mode
	CAN_Config(REQOP_NORMAL);
...
void CAN_SetCNF1(MX_UINT8 VALUE)
{
	...
	MIAC_CS_CAN_CLR_DDR;
	CAN_SPI_SendByte(CAN_WRITE);       	//CAN write mode
	CAN_SPI_SendByte(0x2A);				//start writting at address 0x2A CNF1
	CAN_SPI_SendByte(VALUE);
	//disable SPI
	MIAC_CS_CAN_SET_DDR;
	//Put the CAN device into normal mode
	CAN_Config(REQOP_NORMAL);
