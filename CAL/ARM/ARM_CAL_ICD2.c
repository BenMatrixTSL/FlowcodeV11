// CRC: 
// REVISION: 1.0
// GUID: D214F4AF-1811-42C5-A4AB-B5459D4D0D32
// DATE: 08\08\2022
// DIR: CAL\ARM\ARM_CAL_ICD2.c
/*********************************************************************
 ...
 */
	...
	void MARKER(signed int block_id)
	{
		...
		//{
			...
			ICD_DATA_PIN_WR(0, 1 << ICD_DATA_PIN);			//Data is default low
					...
					while(dbg_hold)
					{
						...
						switch(icd_cmd)
						{
							case ICD_START:									//Start Command
								...
								ICD_DATA_PIN_WR(0, 1 << ICD_CLK_PIN);		//Data is default low
								wait_clock(0, 1);
								break;
	...
	unsigned char icd_byte_tx(char data)
	{
		...
		for(count=0; count<8; count++)
		{
			...
			if(mask)										//Write data onto data line
				ICD_DATA_PIN_WR(1 << ICD_DATA_PIN, 1 << ICD_DATA_PIN);			//Data line as input, allow line to be pulled low by resistor
			else
				ICD_DATA_PIN_WR(0, 1 << ICD_DATA_PIN);
	...
	unsigned char icd_byte_rx(void)
	{
		...
		for(count=0; count<8; count++)				//Receive Byte
		{
			...
			if (ICD_DATA_PIN_RD(1 << ICD_DATA_PIN)) 		//Read incoming data on data pin
				data = data | 0x80;
		...
		ICD_DATA_PIN_WR(0, 1 << ICD_DATA_PIN);			//ICD data 0 send Ack
	...
	unsigned char wait_data(char state)
	{
		...
		while((ICD_DATA_PIN_RD(1<<ICD_DATA_PIN) != state) && (timeout < ICD_TIMEOUT))	//Wait for data
