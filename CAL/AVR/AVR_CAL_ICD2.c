// CRC: 
// REVISION: 1.0
// GUID: D5F6D5FE-9DCD-41EF-89C1-20C90AB9BCE6
// DATE: 08\08\2022
// DIR: CAL\AVR\AVR_CAL_ICD2.c
/*********************************************************************
 ...
 */
	...
	#endif
	#if (ICD_CLK_PORT == 2)
	  #define ICD_CLK_DDR DDRC
	  #define ICD_CLK_READ PINC
	#endif
	#if (ICD_CLK_PORT == 3)
	  #define ICD_CLK_DDR DDRD
	  #define ICD_CLK_READ PIND
	#endif
	#if (ICD_CLK_PORT == 4)
	  #define ICD_CLK_DDR DDRE
	  #define ICD_CLK_READ PINE
	#endif
	#if (ICD_CLK_PORT == 5)
	  #define ICD_CLK_DDR DDRF
	  #define ICD_CLK_READ PINF
	#endif
	#if (ICD_CLK_PORT == 6)
	  #define ICD_CLK_DDR DDRG
	  #define ICD_CLK_READ PING
	#endif
	#if (ICD_CLK_PORT == 7)
	  #define ICD_CLK_DDR DDRH
	  #define ICD_CLK_READ PINH
	#endif
	#if (ICD_CLK_PORT == 8)
	  #define ICD_CLK_DDR DDRI
	  #define ICD_CLK_READ PINI
	#endif
	#if (ICD_CLK_PORT == 9)
	  #define ICD_CLK_DDR DDRJ
	  #define ICD_CLK_READ PINJ
	#endif
	#if (ICD_CLK_PORT == 10)
	  #define ICD_CLK_DDR DDRK
	  #define ICD_CLK_READ PINK
	#endif
	#if (ICD_CLK_PORT == 11)
	  #define ICD_CLK_DDR DDRL
	  #define ICD_CLK_READ PINL
	#endif
	...
	#endif
	#if (ICD_DATA_PORT == 2)
	  #define ICD_DATA_DDR DDRC
	  #define ICD_DATA_WRITE PORTC
	  #define ICD_DATA_READ PINC
	#endif
	#if (ICD_DATA_PORT == 3)
	  #define ICD_DATA_DDR DDRD
	  #define ICD_DATA_WRITE PORTD
	  #define ICD_DATA_READ PIND
	#endif
	#if (ICD_DATA_PORT == 4)
	  #define ICD_DATA_DDR DDRE
	  #define ICD_DATA_WRITE PORTE
	  #define ICD_DATA_READ PINE
	#endif
	#if (ICD_DATA_PORT == 5)
	  #define ICD_DATA_DDR DDRF
	  #define ICD_DATA_WRITE PORTF
	  #define ICD_DATA_READ PINF
	#endif
	#if (ICD_DATA_PORT == 6)
	  #define ICD_DATA_DDR DDRG
	  #define ICD_DATA_WRITE PORTG
	  #define ICD_DATA_READ PING
	#endif
	#if (ICD_DATA_PORT == 7)
	  #define ICD_DATA_DDR DDRH
	  #define ICD_DATA_WRITE PORTH
	  #define ICD_DATA_READ PINH
	#endif
	#if (ICD_DATA_PORT == 8)
	  #define ICD_DATA_DDR DDRI
	  #define ICD_DATA_WRITE PORTI
	  #define ICD_DATA_READ PINI
	#endif
	#if (ICD_DATA_PORT == 9)
	  #define ICD_DATA_DDR DDRJ
	  #define ICD_DATA_WRITE PORTJ
	  #define ICD_DATA_READ PINJ
	#endif
	#if (ICD_DATA_PORT == 10)
	  #define ICD_DATA_DDR DDRK
	  #define ICD_DATA_WRITE PORTK
	  #define ICD_DATA_READ PINK
	#endif
	#if (ICD_DATA_PORT == 11)
	  #define ICD_DATA_DDR DDRL
	  #define ICD_DATA_WRITE PORTL
	  #define ICD_DATA_READ PINL
	#endif
	...
	void MARKER(signed int block_id)
	{
		...
		//{
			...
			clear_bit(ICD_DATA_WRITE, ICD_DATA_PIN);		//Data is default low
			...
			if(dbg_mode)									//Is ICD Active?
			{
				...
				clear_bit(ICD_DATA_WRITE,ICD_DATA_PIN);			//Data output low
			}
	...
	char icd_byte_tx(char data)
	{
		...
		set_bit(ICD_DATA_DDR,ICD_DATA_PIN);					//Data line as output
		clear_bit(ICD_DATA_WRITE,ICD_DATA_PIN);				//Data is 0 RTS
		...
		for(count=0; count<8; count++)
		{
			...
			if(mask)								//Write data onto data line
				set_bit(ICD_DATA_WRITE,ICD_DATA_PIN);
			else
				clear_bit(ICD_DATA_WRITE,ICD_DATA_PIN);
			data = data >> 1;
			if (wait_clock(1<<ICD_CLK_PIN, 1))				//Wait for clock to go high
				return 254;
			if (wait_clock(0, 1))							//Wait for clock to go low
	...
	char icd_byte_rx(void)
	{
		...
		for(count=0; count<8; count++)					//Receive Byte
		{
			...
			if (test_bit(ICD_DATA_READ,ICD_DATA_PIN)) 	//Read incoming data on data pin
				data = data | 0x80;
		...
		clear_bit(ICD_DATA_WRITE, ICD_DATA_PIN);		//ICD data 0 send Ack
